...
<head>
  ...
  <meta property="og:image" content="${origin}assets/jun-grassland.jpg">
  <meta property="og:image:alt" content="${escape(t.photoAlt)}">
  ...
  <link rel="stylesheet" href="${base}home.css?v=20260908">
...
<body id="top">
  ...
  <header class="masthead shell">
    ...
    <nav id="site-nav" class="site-nav" aria-label="${locale === "en" ? "Main navigation" : "主要导航"}">${["curiosities", "reading", "hello"].map((id, i) => `<a href="#${id}">${t.nav[i]}</a>`).join("")}</nav>
  ...
  <main id="main">
    <section class="hero shell" aria-labelledby="hero-title">
      ...
      <div class="hero-actions"><a class="button dark" href="#curiosities">${t.explore}<span aria-hidden="true">↓</span></a><a class="text-link" href="#hello">${t.hello}${arrow}</a></div>
      <figure class="landscape"><div class="landscape-frame"><img src="${base}assets/jun-grassland.jpg" width="1706" height="1279" fetchpriority="high" alt="${t.photoAlt}"></div><figcaption><span>${t.photoNote}</span><span>${t.photoSide}</span></figcaption></figure>
    </section>
    ...
    <section id="curiosities" class="curiosities shell" aria-labelledby="curiosities-title">
      <div class="section-heading"><h2 id="curiosities-title">${t.curiosityTitle}</h2><p>${t.curiosityIntro}</p></div>
      <div class="question-picker" hidden role="group" aria-label="${locale === "en" ? "Choose a curiosity" : "选择一个问题"}">${t.questions.map((q, i) => `<button type="button" data-question="${i}" aria-pressed="${i === 0}" aria-controls="question-${i}">${q.topic}<span aria-hidden="true">↗</span></button>`).join("")}</div>
      <div class="question-deck">${t.questions.map((q, i) => `<article class="question-card tone-${i}" id="question-${i}" aria-labelledby="question-title-${i}"><div class="question-copy"><p class="eyebrow">${q.topic}</p><h3 id="question-title-${i}">${q.title}</h3><p>${q.text}</p><a class="text-link question-reading" href="#${q.target}">${q.link}${arrow}</a></div><div class="orbit-art orbit-${i}" aria-hidden="true"><div class="orbit-ring ring-one"></div><div class="orbit-ring ring-two"></div><div class="orbit-ring ring-three"></div><span class="orbit-center">${i === 0 ? "↻" : i === 1 ? "✳" : "↔"}</span>${q.words.map((w, n) => `<span class="orbit-word orbit-word-${n}">${w}</span>`).join("")}</div></article>`).join("")}</div>
    </section>
    <section id="reading" class="reading shell" aria-labelledby="reading-title"><div class="section-heading"><h2 id="reading-title">${t.readingTitle}</h2><p>${t.readingIntro}</p></div>
      <div class="reading-tools"><div class="filters" hidden role="group" aria-label="${locale === "en" ? "Filter papers by topic" : "按话题筛选论文"}">${["all", "learning", "knowledge"].map((filter, i) => `<button type="button" data-filter="${filter}" aria-pressed="${i === 0}">${t.filters[i]}</button>`).join("")}</div><span id="reading-count" role="status" data-suffix="${t.results}">${papers.length} ${t.results}</span></div>
      <div class="paper-list">${papers.map((p, i) => `<article class="paper" id="paper-${p.id}" data-topic="${p.topic}"><div class="paper-index" aria-hidden="true">${String(i + 1).padStart(2, "0")}</div><div class="paper-content"><p class="paper-meta"><span lang="en">${escape(p.journal)}</span><span>${p.year}</span></p><h3 lang="en"><a href="${p.doi}" target="_blank" rel="noopener noreferrer">${escape(p.title)}</a></h3><p class="paper-note">${escape(p[locale])}</p></div><a class="paper-arrow" href="${p.doi}" target="_blank" rel="noopener noreferrer" aria-label="${escape(t.paperLink + ": " + p.title)}">${arrow}</a></article>`).join("")}</div>
  ...
  <script src="${base}home.js?v=20260908" defer></script>
</body>
</html>
`;
}
