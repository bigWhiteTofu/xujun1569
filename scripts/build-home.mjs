...
<body id="top">
  ...
  <main id="main" tabindex="-1">
    <section id="home" class="hero shell page-screen" data-label="${t.nav[0]}" tabindex="-1" aria-labelledby="hero-title">
      <div class="hero-copy"><p class="eyebrow">${t.intro}</p>
      <h1 id="hero-title">${t.hero[0]}<br><em>${t.hero[1]}</em></h1>
      <p class="hero-lead">${t.lead}</p><p class="hero-bio">${t.bio}</p>
    ...
    <section id="research" class="research-atlas shell page-screen" data-label="${t.nav[1]}" tabindex="-1" aria-labelledby="research-title">
    ...
    <section id="field" class="field-note shell page-screen" data-label="${t.deck.field}" tabindex="-1" aria-labelledby="field-title"><div class="field-copy"><p class="eyebrow">${locale === "en" ? "BEYOND THE SCREEN" : "屏幕之外"}</p><h2 id="field-title">${a.fieldTitle}</h2><p>${a.fieldText}</p></div><figure class="landscape"><div class="landscape-frame"><img src="${base}assets/jun-grassland.jpg" width="1706" height="1279" loading="lazy" alt="${t.photoAlt}"></div><figcaption>${t.photoSide}</figcaption></figure></section>
    <section id="hello" class="contact page-screen" data-label="${t.nav[3]}" tabindex="-1" aria-labelledby="hello-title"><div class="shell contact-grid"><div class="contact-copy"><p class="eyebrow">${locale === "en" ? "KEEP THE CONVERSATION GOING" : "让对话继续"}</p><h2 id="hello-title">${t.contactTitle}</h2><p>${t.contactIntro}</p><a class="email-address" href="mailto:xujun1569@gmail.com">xujun1569@gmail.com ${arrow}</a><div class="contact-links"><button id="copy-email" type="button" hidden>${t.copy}</button><a href="https://orcid.org/0000-0002-0557-2046" target="_blank" rel="noopener noreferrer">ORCID ${arrow}</a></div><p id="copy-status" class="status" role="status"></p></div>
