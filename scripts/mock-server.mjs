...
const visits = [];
const messages = [];
const types = { ".html": "text/html; charset=utf-8", ".css": "text/css; charset=utf-8", ".js": "text/javascript; charset=utf-8", ".jpg": "image/jpeg", ".png": "image/png", ".pdf": "application/pdf", ".svg": "image/svg+xml", ".woff2": "font/woff2", ".xml": "application/xml" };
...
http.createServer(async (request, response) => {
  ...
  if (url.pathname === '/xujun1569') { response.writeHead(301, { location: '/xujun1569/' + url.search }); return response.end(); }
  const sitePath = url.pathname.replace(/^\/xujun1569\//, '/');
  const requested = decodeURIComponent(sitePath.slice(1));
  let file = normalize(join(root, requested));
  const local = relative(root, file);
  if (local.startsWith('..') || isAbsolute(local)) return send(response, 403, "Forbidden", "text/plain");
  try {
    if ((await stat(file)).isDirectory()) {
      if (!url.pathname.endsWith('/')) { response.writeHead(301, { location: url.pathname + '/' + url.search }); return response.end(); }
