...
The public page presents one screen at a time: chalkboard portrait and student introduction → research portrait/word cloud → four expanded research directions → paginated publications → grassland photograph → conversation. The header and footer remain visible. Wheel gestures, vertical swipes, keyboard paging, and previous/next controls turn pages. Long content remains scrollable within the current screen, with a fresh swipe needed to leave that screen. The atlas reuses `assets/research-wordcloud.png` without altering the asset or application page. Its expandable topic descriptions do not link to papers.
